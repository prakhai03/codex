...
const stdoutChangeCallbacks = new Set();

export function onStdoutChange(callback) {
    stdoutChangeCallbacks.add(callback);
    return () => stdoutChangeCallbacks.delete(callback);
}
...
function handleResult(data) {
    ...
    stdoutChangeCallbacks.forEach(callback => callback(output));
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    require(["vs/editor/editor.main"], function (ignorable) {
        ...
        layout.registerComponent("chatbot", function (container, state) {
            // Create a div element for the chatbot
            const chatbotElement = document.createElement('div');
            chatbotElement.style.width = '100%';
            chatbotElement.style.height = '100%';
            chatbotElement.style.overflow = 'auto';
            container.getElement().append(chatbotElement);
            
            // Initialize the chatbot in the created element
            initChatbot(chatbotElement);
        });
